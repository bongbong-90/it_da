...
const CATEGORY_BASE_DATA: Record<CategoryType, { icon: string; description: string; subcategories: { name: string }[] }> = {
    '스포츠': {
        icon: '🏃',
        description: '건강한 몸과 마음을 위한\n다양한 운동 모임',
        subcategories: [
            { name: '러닝' }, { name: '축구' }, { name: '배드민턴' }, { name: '등산' },
            { name: '요가' }, { name: '사이클링' }, { name: '클라이밍' }
        ]
    },
    '맛집': {
        icon: '🍴',
    ...
    '문화예술': {
        ...
        description: '감성 충전하는\n전시회와 공연 모임',
        subcategories: [
            { name: '전시회' }, { name: '공연' }, { name: '갤러리' },
            { name: '공방체험' }, { name: '사진촬영' }, { name: '버스킹' }
        ]
    },
    '스터디': {
        icon: '📚',
        description: '함께 성장하는\n학습과 자기계발',
        subcategories: [
            { name: '영어회화' }, { name: '독서토론' }, { name: '코딩' },
            { name: '재테크' }, { name: '자격증' }, { name: '세미나' }
        ]
    },
    '취미활동': {
        icon: '🎪',
        ...
        subcategories: [
            { name: '그림' }, { name: '베이킹' }, { name: '쿠킹' },
            { name: '플라워' }, { name: '캘리그라피' }, { name: '댄스' }
        ]
    },
    '소셜': {
        icon: '💬',
        ...
        subcategories: [
            { name: '보드게임' }, { name: '방탈출' }, { name: '볼링' },
            { name: '당구' }, { name: '노래방' }, { name: '와인바' }
        ]
    }
};
...
const CategoryListPage = () => {
    ...
    const getStats = (categoryName: string) => {
        const stats = categoryStats[categoryName] as CategoryDetailStatsItem | undefined;
        if (stats) {
            return {
        ...
        <div className="category-list-page">
            ...
            <div className="main-container">
                ...
                <div className="category-grid">
                    {(Object.entries(CATEGORY_BASE_DATA) as [CategoryType, typeof CATEGORY_BASE_DATA[CategoryType]][]).map(
